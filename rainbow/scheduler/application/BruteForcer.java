...
public class BruteForcer extends Thread {
	...
	public BruteForcer(NewQuery query, WorkBlockSetup work) {
		this.query = query;
		this.work = work;
		try {
	...
	public void run() {
		...
		for (long blockNumber = work.getStartBlockNumber(); blockNumber < work.getEndBlockNumber(); blockNumber++) {
			...
			for (int blockIndex = 0; blockIndex < space.BLOCK_SIZE; blockIndex++) {
				md.reset();
				md.update(space.getText(blockIndex).getBytes());
				if (check(md.digest())) {
					System.out.println("Found match " + space.getText(blockIndex));
					return;
				}
			}
